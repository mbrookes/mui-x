...
| `shared/columnValidation.ts`        | `checkColumnAgainstAllowlist`, `validateDescriptorColumns`, `validateHavingAliases`, `validateAggregationAliases` — fail-closed column-allowlist, HAVING-alias, and aggregation-alias checks shared by the read (`handler.ts`) and write (`mutationBuilder.ts`) paths |
