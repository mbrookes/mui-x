...
**Every foreign-source read in the package goes through L1**, not `dataSources[id]?.rows` directly — L4's anchor/junction/M:N-remote rows, `dataSourceGraph`'s related-field enrichment (one-hop and two-hop), `crossSourceEnrichment`, `grainResolution.enrichForeignExpressionFields`, and the CSV export. Two failure modes justify this:
...
- **All four `resolveChartRowsForAggregation` sites thread the same three things**: the resolved `widgetFilters` (so an anchor-scoped filter is re-applied at L4, not just semi-joined at L3), `crossFilterAllPages`, and `includeWidgetRank` via `shouldApplyWidgetRankAtL3`. Every omission produced the same class of bug — resurrected anchor rows, an out-of-scope cross-page cross-filter, or a previous period compared unranked against a ranked current period.
...
- **Every filter read goes through `selectFiltersForWidget`** with the same `crossFilterAllPages` — trend, sparkline, and the hover filter-subtitle alike — so a disabled or off-page filter can neither drive the value nor be listed as applying.
...
- `MapSetupPanel` deliberately offers value fields from related sources, so `regionData` FK-dedupes them (`buildManyToOneRelationshipIndex` + a per-region `Set` of normalized FK keys) — the map analogue of the grid's `symmetricAggregate`. A same-source field or unresolvable FK falls back to a plain per-row reduce, already correct there.
...
Three cases stay server-side despite a subtler difference, each with a one-time `warnAdapterDivergence` rather than silent divergence, because routing a common high-selectivity predicate client-side would defeat the pushdown (and drop the filter entirely for an already-aggregated widget): `not_equals` (SQL three-valued logic excludes NULLs; the evaluator keeps them) and `equals`/inequalities on a `date`/`datetime` field (the server compares the raw column to a bare date string while in-memory normalizes both sides). Also warned rather than silent: a filter on an arithmetic expression field (no server column exists; `resolve()` reports `{ skip: true }`), and `count_distinct` downgrading to a plain `count`.
