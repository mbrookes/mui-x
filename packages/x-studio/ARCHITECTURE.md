...
- **Layout math**: the live canvas renders a 24-column grid (`canvasGridConstants.ts`'s `GRID_COLS = 24`), and the drag-resize handle commits spans in 24-column units via `setAdjacentWidgetColSpans`. `setWidgetColSpan` and `setWidgetColSpanInRow` instead clamp to a 3–12 range and rebalance siblings so a resize never exceeds 12 columns — a second, currently-unreconciled unit system with no production call site of its own (only the AI `set_widget_width` tool, via the schema package's `applyMutation.ts` handler, and tests, exercise the 12-column path). See `ARCHITECTURE_REVIEW.md` for the resulting AI-vs-drag layout-corruption risk.
...
- **Root components**: `Studio` (full authoring UI, imperative `StudioHandle` ref), `StudioDashboard` (embed-first, view-oriented wrapper over `Studio` that takes a pre-built `config: StudioState` + `dataAdapters` map, for displaying a live dashboard without exposing the authoring UI — intended to reload on a `config` reference change via `StudioHandle.loadSerializedState`, though see `ARCHITECTURE_REVIEW.md` for a known defect in that reload path).
