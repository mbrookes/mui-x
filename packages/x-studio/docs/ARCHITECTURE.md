...
**Keys are only half the problem: closed-union config *values* need the same guard.** A record indexed by a value that TypeScript types as a closed union, but that nothing validates at the load/AI-tool boundary, is the identical bug wearing different clothes — the config-key validation in `StudioController` (§2.4) only checks that a key *name* is legal for the widget's kind, never that its *value* is one of the declared members. `StudioMapWidget`'s `COLOR_RAMPS[colorScheme]` (§10.7) and `textFontFamily.ts`'s `NAMED_FONT_STACKS[value]` (§10.1) were both of this shape, and the ghost-series `seriesData[name]` lookups in `StudioBarChart`/`StudioLineAreaChart` (§10.3) are the silent-wrong-value variant, where a split-by *category value* drawn from row data is the key.
