...
function In(e, t, n, r) {
  ...
  for (let l = 0; l < o; ++l) {
    ...
    for (let v = 1; v < T; ++v) {
      j[w + v] = e() < 0.7 ? L[I(e, 0, L.length)] : I(e, 0, y);
    }
    w += T;
  }
  w = 0;
...
function Un(e) {
  ...
  let r = 0;
  for (const i of e) {
    i == null ? ++r : n.has(i) || (n.set(i, t.length), t.push(i));
  }
  const o = t.length,
...
async function tr(e) {
  ...
  const u = (c) => {
    ...
    i = new Map();
  for (const c of r) {
    i.set(String(c.product_id), Number(c.unit_cost ?? 0));
  }
  const g = (c) => {
      const y = Number(c.quantity ?? 0),
    ...
    M = new Map();
  for (const c of r) {
    M.set(String(c.product_id), String(c.subcategory));
  }
  const m = new Map(),
    d = new Map();
  for (const c of n) {
    ...
    if (rr(u)) {
      ...
      if ((n.push(i), i instanceof Float64Array)) {
        ...
        if (g > 0) {
          ...
          (r.push(null), o.push({ nullCount: 0 }));
        }
      } else {
        (r.push(null), o.push(null));
      }
    }
  }
