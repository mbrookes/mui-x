...
- **User-driven methods that delegate** through `commitMutation` today: `addWidget`, `removeWidget`, `setWidgetLayout`, `updateWidgetConfig`, `addFilter`, `removeFilter`, `addPage`, `removePage`, `renamePage`, `setDashboardTitle`, `setActivePage`. Each still owns the argument-shaping and validation that isn't the reducer's job — `addFilter` stamps a page filter with the active page, `setWidgetLayout` throws on unknown/orphaned widget ids before delegating, `addPage` generates the page id, `setActivePage` keeps its same-value early return and stays non-undoable/unlogged, and `addWidget`/`removeWidget`/`updateWidgetConfig` pass a `transform` for their shell/title side effects.
...
- **Methods with no shared `StateMutation` equivalent are permanently controller-owned by design, not a migration gap**: shell/selection (`setSelectedWidget`, `selectField`, `clearSelection`, drawer toggles), data-source injection (`upsertDataSource`, `setDataSourceRows`, `setDataSourceAdapter`), transient interactive filters (`applyInteractiveFilter`, `clearInteractiveFilter`), cross-filters (`applyCrossFilter`, `clearCrossFilter`, `clearAllCrossFilters`), filter presets (`saveFilterPreset`, `applyFilterPreset`, …), relationships, expression fields, date-range filter building, and composite operations like `duplicateWidget`, `moveWidgetToPage`, and `reorderPages`. These either touch React/runtime-only state the dependency-free reducer must never own, or are multi-step compositions with no single `StateMutation` variant (and none will be added — see D7 in the conversion brief).
- **Known tracked gap (out of scope here)**: the pointer drag-and-drop widget move in `StudioCanvas.tsx` still commits layout via its own `controller.updateState({ pages, shell })` path — a third, independent implementation of layout semantics that does _not_ yet route through `commitMutation`/the reducer. Converging it is a separate, larger follow-up.
...
- `pruneWidgetColSpan(colSpans, widgetId)` — removes a widget's entry from a page's col-span map; used by the drop handler on every cross-page widget move (both the source and destination page's maps) so a moved widget's stale span never lingers on a page it no longer belongs to.
