...
✅ BL-189: Add a CI step that typechecks the example apps. Example type errors never surfaced in the package typecheck gate (this is how a half-finished map migration sat type-clean at the package level while broken in the example).

**Done** — **all x-studio example variants except `x-studio-ag` are now installable, gated, and green** (`x-studio`, `x-studio-composed`, `x-studio-ai`, `x-studio-dev-server`, `x-studio-ai-proxy`; membership is `examples/x-studio*` minus `!examples/x-studio-ag`). `x-studio-ag` (ag-grid) has genuine errors → **BL-193**. The prior batch's "just add to the workspace" premise was necessary but not sufficient; the real blocker was deeper:
...
- **The other variants** (follow-up commit): `x-studio-composed` (1324→0) and `x-studio-dev-server` (91→0) had the identical incomplete-tsconfig cascade — fixed with the same `extends ../../tsconfig.json` + shim recipe. `x-studio-ai` (6→0) uses deliberate hand-written `.d.ts` stubs (kept that design); its stubs had just drifted — added the `de/es/frLocaleText` (x-studio) and `deDE/esES/frFR` (date-pickers locales) exports that the real packages provide. `x-studio-ai-proxy` was already 0, only needed membership.

✅ BL-193: Bring `examples/x-studio-ag` into the BL-189 typecheck gate. It was the one x-studio example still excluded from `pnpm-workspace.yaml` (`!examples/x-studio-ag`), assumed to have **genuine** ag-grid integration type errors (≈1306).

**Done** (a694b7d0e9): the prior diagnosis was wrong — **all 1306 errors were the same incomplete-tsconfig cascade** as the other variants, not genuine ag-grid drift. `x-studio-ag` consumes ag-grid only via the pre-built `ag-studio-react` package (which ships its own `.d.ts`, so it is _not_ typechecked from source); the example's own `src/**` had **zero** real type errors. Applying the standard `extends ../../tsconfig.json` recipe + the `typecheckAugmentations.ts` shim took it **1306 → 0** with no source edits and no `@ts-expect-error`. Dropped the `!examples/x-studio-ag` exclusion (membership is now plain `examples/x-studio*`); `cd examples/x-studio-ag && pnpm exec tsc --noEmit -p tsconfig.json` → 0. Sanity-checked `x-studio-composed` (still 0) and `pnpm --filter @mui/x-studio typescript` (still 0). No deps changed.
...
**Done** (500277ecdd): the real count was **158 errors** (the ~68 estimate undercounted; the spread was JSDoc-dominated, not quote-dominated), now **0**. `npx eslint packages/x-studio/src --ext .ts,.tsx` → 0 errors; `pnpm --filter @mui/x-studio typescript` clean; 1078 unit tests pass (locale `locales.test.ts` unchanged — translation strings byte-for-byte untouched, verified via empty `git diff` on `src/locales/`).
...
✅ BL-193: The x-studio unit suite was **systemically flaky** — across identical runs it produced anywhere from 0 to 34 failures (and the prior backlog's "N tests pass" claims were lucky green runs). Plus 4 deterministic failures in `generateInsight.test.ts` and 12 eslint errors that contradicted BL-187/188/190's green claims (lint/tests had regressed since).

**Root cause (flakiness)** — the repo runs vitest with `isolate: false` (`vitest.shared.mts`), so the module registry is shared across test files in a worker. **10** test files each `vi.mock('.../context')` with a factory closing over their own module-level `mockState`/`controller`. Source modules (`useChartWidgetData.ts`, etc.) load once and bind to whichever file's factory registered **first**; a second file mocking the same module then silently read the first file's state. Deterministic repro: run `useChartWidgetData.test.tsx` + `StudioChartWidget.test.tsx` in one worker → **whichever runs second fails** (the first's `mockState` leaks in). The erratic full-suite count came from which files landed in which worker, in what order.

**Fixed:**

1. **Test isolation** — added `packages/x-studio/test/studioContextMock.ts`: stable shared `vi.fn()`s (`mockUseStudioSelector`/`mockUseStudioController`) that read from a per-file holder via `configureStudioContextMock({ getState, controller | getController })`, re-applied in each `beforeEach` (so it survives files whose `afterEach` runs `vi.restoreAllMocks()`). All 10 `context`-mocking files now route through it and override **both** hooks uniformly (so the binding can't leak a real hook to a file that needs it mocked). `tsconfig.json` include widened to `test/**/*`. The shared `import` must precede any import that transitively loads `context` (else the hoisted `vi.mock` factory references uninitialised fns — caught in `renderPerf.test.tsx`); files that reassign their controller mid-test (`TabbedSidebar`, `renderPerf`) use `getController`. Verified: **17 consecutive fully-green full-suite runs** (default parallelism + the previously-worst `--no-file-parallelism`, which used to fail 10–34), vs ~50% failing before.
2. **`generateInsight.test.ts` (4 deterministic failures)** — stale tests, not a regression. The insight builder was refactored (commit `fd15c852`) to dispatch chart/KPI/map widgets to type-specific summaries (`buildChartWidgetSummary` → "Aggregated by month…"); the generic raw-row path (`Data sample (…)` + `aggregateRows` "aggregated buckets") is now only reached by grid/pivot/scatter/gantt/sankey. The 4 tests asserted the raw-row output but used a `chart` widget; repointed them at a grid widget (`GRID_OVERRIDE`) so they exercise the machinery they intend to test.
3. **eslint (12 errors)** — `no-shadow` (`legendMaxHeight`/`mapTranslate` inner locals in `StudioMapWidget.tsx`), `no-nested-ternary` (`generateInsight.ts`, `WidgetTypeCard.tsx`, `StudioMapWidget.tsx` legend `alignSelf` → object lookup), an unused `cursor` var (`WidgetTypeCard.tsx`, was shadowed by an inline nested ternary — de-nested and wired up), and `curly`/`prefer-template` (auto-fixed). `npx eslint packages/x-studio/src` → 0.

**Note for contributors:** with `isolate: false`, any test mocking a shared x-studio module (`context`, etc.) must route through a stable shared mock + per-file holder — a per-file `vi.mock` factory closing over local `let mockState` will leak across files. **Environment:** the gates only run on Node ≥ 22.22.3 (the package `engines` floor); a container on 22.22.2 can't start `pnpm` at all.

---

## Follow-ups surfaced during the KPI target / metric-ref cleanup (2026-06-15)

✅ BL-194: Change KPI target config from a data-source reference (`kpiTargetRef?: StudioMetricRef`) to a discrete configurable value (`kpiTargetValue?: number`). Make target required for gauge (always shown, no toggle); use that value to replace gauge max. Remove gauge min (it was not wired to anything useful).

**Done**: Replaced `kpiTargetRef` + `kpiSparklineGaugeMin`/`kpiSparklineGaugeMax` with a single `kpiTargetValue?: number` field. `KpiSparkline` hardcodes `valueMin={0}`, uses `kpiTargetValue` as `gaugeMax` with a `safeMax` guard (`max(1, value)` when unset or ≤ 0). `KpiSparklineOptions` renders a single Target `TextField` (always visible for gauge). `KpiSetupPanel` target section uses a `TextField` for `kpiTargetValue`. Removed `kpiSetupMinLabel`/`kpiSetupMaxLabel` locale keys; updated `kpiSetupTargetHelperText`. Updated `salesDashboard.ts` example config.

✅ BL-195: Remove all `StudioMetricRef` / business-metrics / `resolveMetricRef` / `resolveMetricRefs` functionality. This feature (filter values dynamically driven by a row in a "business metrics" data source) was complex, not currently used in any example, and added significant surface area to the filter pipeline, UI, and locale layer.
