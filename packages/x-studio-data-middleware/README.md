...
| `options.queryTimeoutMs`              | `number`                   | Default: `30_000` (30s). Per-query statement timeout applied to the preflight `COUNT(*)` and the data query alike, so a slow query cannot pin a pooled connection and starve the host app's own traffic. Set `0` to opt out (for example, when the database enforces `statement_timeout`). Also available on `handleMutation`'s options. |
