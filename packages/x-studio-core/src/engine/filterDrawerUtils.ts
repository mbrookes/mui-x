...
import { hasBetweenBound, isConditionComplete } from './filterUtils';
 ...
 * `between` ↔ scalar reset guard is written as `… && !isStrictRelativeDateValue(value)`, so that one
 ...
 *
 * NOTE: `filterUtils.ts` carries a deliberately LOOSE `isRelativeDateValue` for the evaluation
 ...
 * it must be the strict of the two. The names used to be identical, which was survivable while
 * they sat in different directories and became an ambiguous re-export the moment the engine grew
 * a barrel — hence `isStrict…` here.
 */
export function isStrictRelativeDateValue(value: unknown): value is RelativeDateValue {
...
export function isBetweenShapedValue(value: unknown): boolean {
  return (
    ...
    typeof value === 'object' &&
    !Array.isArray(value) &&
    !isStrictRelativeDateValue(value)
  );
}
...
function formatFilterValue(
  ...
  if (isStrictRelativeDateValue(value)) {
