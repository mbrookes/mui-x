...
import { fillTemporalLabelGaps, normalizeToDate } from './temporalUtils';
import { resolveDateRangePreset } from './filterUtils';
import { aggregateCellValues } from './aggregate';
import { getStudioLocale } from './studioLocale';
import { evaluateMeasure } from '../utils/expressionEvaluator';
import { lookup } from '../utils/safeLookup';
import { isStrictRelativeDateValue, relativeToAbsolute } from './filterDrawerUtils';
...
export function extractDateRange(filter: StudioFilterState): { start: Date; end: Date } | null {
  const toDate = (v: unknown): Date | null => {
    ...
    const str = isStrictRelativeDateValue(v) ? relativeToAbsolute(v) : (v as string);
