 ...
 *
 * It is NOT the only site (this comment used to say `internals/widgetLayoutMove.ts` (keyboard move)
 * and `StudioController.duplicateWidget` "each USED TO re-derive their own copy"; both still do, as
 * `Math.floor(GRID_COLS / MIN_SPAN)` and `Math.floor(GRID_COLS / MIN_SPAN_COLS)` respectively).
 * There is no functional drift and there cannot be: all three derive from the same
 * `GRID_COLS`/`MIN_SPAN` exported by `@mui/x-studio-schema`, never from a literal. The two
 * non-canvas sites re-derive rather than import this module because it pulls in React and the
 * drag-session types they have no reason to depend on. Keep every site derived; a fourth must
 * derive from the schema constants too.
 */
