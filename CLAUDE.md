...
- `doc: StudioDoc` — the user-authored dashboard document, and the ONLY partition that is persisted, undoable, and mutated by the shared `applyMutation` reducer: `dashboard` (title, date range, theme), `pages`, `widgets` (flat, not nested per-page), `semanticModel` (the named model holding `relationships` and `expressionFields` — ADR 0004; read the EFFECTIVE model via `resolveSemanticModel(state)`, which honours a host override in `runtime.semanticModels`), `filters` (including cross-filter entries — those live here, not in `session`, because the reducer manipulates them and `applyCrossFilter` is deliberately undoable; they're stripped only at the persistence boundary), `filterPresets?`, `ai?` (chat thread state).
