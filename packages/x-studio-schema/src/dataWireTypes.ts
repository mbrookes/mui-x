/**
 * THE BATCH-QUERY WIRE PROTOCOL — the one definition, for both sides of it.
 *
 * `@mui/x-studio`'s `createBatchingAdapter` builds these shapes; `@mui/x-studio-data-middleware`'s
 * `handleBatchQuery` validates and executes them. Neither package may depend on the other — the
 * client must not pull in a Node-only, Knex-peered package, and the server must not pull in React
 * — so before this module they each declared the protocol independently and kept the two in
 * agreement by hand, with mirror assertions that each package asserted in its OWN suite. Neither
 * suite can see the other side of the wire, so that arrangement could not fail until it was too
 * late, and it repeatedly didn't: emitted aliases the middleware rejects fail-closed, join `on`
 * pairs in the orientation the middleware does not accept, batches over the server's per-request
 * widget cap, and a hand-copied `MAX_WIDGETS_PER_BATCH`.
 *
 * They already share this package, which is zero-dependency and therefore importable from a
 * browser bundle and a Node server alike — the same arrangement the AI protocol has always had
 * (`aiTypes.ts`, shared by `@mui/x-studio` and `@mui/x-studio-ai-middleware`), and which produced
 * no equivalent class of defect. This module closes the asymmetry.
 *
 * RULE: every type here is part of the HTTP payload. A shape that is internal to one side —
 * a compiled security policy, a validated query plan, an adapter's in-flight bookkeeping —
 * does NOT belong here, and stays in the package that owns it.
 */
...
export interface WidgetQueryResult {
  ...
  /**
   * Routing tier that served this widget (corrected to match `executeForTier`'s actual behavior):
   * 'client' — raw filtered rows returned; the client filters/aggregates in-browser 'server' — raw
   * filtered rows returned; the middleware caches them for reuse
...
/**
 * Hard ceiling on the number of items (widgets, mutations) ONE batch request may carry.
 *
 * A wire constraint, not a server implementation detail, which is why it lives here: the client
 * has to CHUNK against it and the server REJECTS against it, and the two numbers must be the same
 * number. They were not — `@mui/x-studio` carried a hand-kept copy — and because the middleware
 * rejects an over-cap batch WHOLE, before its per-widget loop, one widget past the cap failed
 * every other widget on the page with a bare 500 rather than an actionable per-widget error.
 *
 * `@mui/x-studio-data-middleware` re-exports this as `MAX_WIDGETS_PER_BATCH` (queries) and
 * `MAX_MUTATIONS_PER_BATCH` (mutations); both bound the same wire position and have always held
 * the same value.
 *
 * The middleware's OTHER bounds — per-descriptor array sizes, identifier and value lengths,
 * semi-join depth — stay in that package. They are rejection rules for a payload the client does
 * not generate anywhere near, so sharing them would widen this module without closing any gap.
 */
