...
productId: x-studio
packageName: '@mui/x-studio-data-middleware'
...
```bash pnpm
pnpm add @mui/x-studio-data-middleware knex lru-cache
```

```bash yarn
yarn add @mui/x-studio-data-middleware knex lru-cache
```
...
```ts
import { LRUCacheProvider } from '@mui/x-studio-data-middleware';
...
```ts
import type { CacheProvider, CacheEntry } from '@mui/x-studio-data-middleware';
