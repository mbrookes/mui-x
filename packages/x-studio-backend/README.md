...
| **Production — full backend**       | **Use `@mui/x-studio-backend`** — key and all AI logic stay server-side           |
...
On the client, set `mode: 'x-studio-backend'` in `StudioAIConfig`:
...
const aiConfig: StudioAIConfig = {
  ...
  mode: 'x-studio-backend',  // switches to the backend protocol
};
...
The client adapter (`mode: 'x-studio-backend'`) handles all these automatically.
...
```ts
import { runAgenticLoop } from '@mui/x-studio-backend';
...
- [`@mui/x-studio-middleware`](../x-studio-middleware/README.md) — SQL data pipeline handler
