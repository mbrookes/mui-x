...
| Bounds                | `DEFAULT_QUERY_TIMEOUT_MS`, `MAX_WIDGETS_PER_BATCH`, `MAX_MUTATIONS_PER_BATCH` — exported so a host that assembles batches itself chunks against the real cap instead of hard-coding it. An over-cap batch is rejected *whole*, before the per-item loop, so getting it wrong fails every item at once.                                                                                                                              |
...
| `security/compileSecurityPolicy.ts` | `compileSecurityPolicy`, `SecurityPolicyOptions`, `CompiledSecurityPolicy`, `SINGLE_TENANT_POLICY_DIGEST`. (`computePolicyDigest` is module-PRIVATE: the digest is only ever read off a `CompiledSecurityPolicy`, which is what keeps the read and write paths computing it identically.)                                                                                 |
...
**Table allowlist (Zero-Knowledge Rule).** `assertTablesAllowed()` checks `widget.table` and every `widget.joins[].table` against `options.schemaAllowlist`. `assertQualifiedColumnsAllowed()` then extends the identical rule to every table-**qualified** column reference in `columns`, `filters[].column`, `orderBy[].column`, the physical side of `columnAliases`, `aggregations[].column`, both sides of every `joins[].on` pair, and — walked **recursively**, so a nested semi-join's references are covered too — every `semiJoins[].column`, `semiJoins[].foreignColumn` and `semiJoins[].filters[].column`. (`semiJoins[].table` itself is covered by `assertTablesAllowed` via `collectSemiJoinTables`.) The semi-join arm is not incidental: a subquery reference naming an unlisted table was the one shape that skipped this unconditional check on a `schemaAllowlist`-only deployment.
...
| `aggregations: PlanAggregation[]`    | `physical`/`func`/`alias`.                                                                                                                                                  |
...
**A `columns` entry that the descriptor also aggregates is a MEASURE, not a dimension** — it is projected only inside its aggregate clause and never joins the GROUP BY. Membership is decided by the column itself (`aggregations[].column`, compared on primary-table-qualified physicals so a qualified `orders.amount` and an unqualified projected `amount` still match), never by how the alias is spelled.

The plan used to carry a precomputed `pureMeasure: agg.alias === resultKeyOf(physical)` flag, and `execute.ts` kept a column out of GROUP BY only when it was set — an alias-NAME heuristic standing in for "is this column aggregated". It was wrong in both directions (F1). Overt half: the exact shape `security/queryTypes.ts` documents — `columns: ['category', 'amount']` with `{ column: 'amount', func: 'sum', alias: 'total_revenue' }` — emitted `group by "orders"."category", "orders"."amount"` on all three dialects, returning one row per (category, amount) with a per-VALUE total instead of one row per category. Well-formed SQL, silently the wrong grain, visible only once a measure has more than one distinct value inside a group; renaming the alias to `amount` was the only way to get the right answer. Fail-closed half: a projected expression field aggregated under its own logical id (`columnAliases: { 'expr-1': 'orders.amount' }`, `alias: 'expr-1'`) compared the alias against the PHYSICAL column's last segment, so it did not count as a measure, stayed in the projection-key list, and the widget was hard-rejected with `Aggregation alias "expr-1" collides with a projected column.` The flag is gone; `execute.ts` and `validateQueryPlan` both key off membership in `aggregations`, so there is no derived value left to drift from the question it stands for.

A consequence worth stating: a descriptor whose ONLY projected column is the one it aggregates has no dimension left and is a **global** aggregate. "How many rows per group" is spelled by counting a NOT NULL column that is not the group key — this protocol has no `COUNT(*)` (see [Wildcards and projection keys](#wildcards-and-projection-keys)).
...
2. **`validateWildcardProjection`**, then **`validateAggregationColumns`** — see [Wildcards and projection keys](#wildcards-and-projection-keys).
...
4. **`validateAggregationAliases`** — every `aggregations[].alias` must match `SAFE_ALIAS_PATTERN` (`[A-Za-z0-9_-]+`), be no longer than `MAX_STRING_LENGTH`, and be **unique** — including against every projected column's result-row key. Two aggregations sharing an alias both pass the charset check but `execute.ts` SELECTs them under the same row key (one silently dropped) and `applyHaving` binds to whichever `find` returns first. A projected column the descriptor AGGREGATES is excluded from that key list, since `execute.ts` projects it only inside the aggregate clause and it therefore yields no separate result-row key; membership is compared on primary-table-qualified physicals so an unqualified column and its qualified aggregation still match.
...
- **`validateAggregationColumns`** rejects a wildcard in `aggregations[].column` (F3). `validateWildcardProjection` reads `descriptor.columns` ONLY, so `{ column: '*', func: 'count', alias: 'n' }` slipped past every check and reached `execute.ts`'s `qualify()`, emitting `count("orders".*)` on all three dialects — a syntax error on SQLite (`near "*": syntax error`) and MySQL, both then masked by `sanitizeBoundaryError` into the generic per-widget error, and on PostgreSQL a **valid** composite-type argument answering a different question than the one asked. Only a concrete `columnAllowlist` rejected it before: a `schemaAllowlist`-only deployment has no column list to fail against, and `columnAllowlist: { orders: ['*'] }` admits the literal `"*"`. **`COUNT(*)` is not expressible in this protocol** — count a NOT NULL column instead.
...
- **`'db'` with aggregations** — splits `queryPlan.columns` into GROUP BY dimensions vs measures — a projected column is a measure exactly when `aggregations[]` aggregates it (F1), never by how its alias is spelled — comparing **primary-table-qualified physicals on both sides** so a qualified measure (`orders.amount`) and an unqualified projected column (`amount`) still match and the measure stays out of GROUP BY. Then, per aggregation:
...
| :--------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Count**        | `MAX_WIDGETS_PER_BATCH` / `MAX_MUTATIONS_PER_BATCH` (50)                                                                                                                                | Items per request. Both re-exported from `index.ts`. `@mui/x-studio`'s `MAX_BATCH_WIDGETS_PER_REQUEST` is a hand-kept COPY of the first — that package must not depend on this Node-only, Knex-peered one — pinned equal by a mirror assertion in each package's own suite (`__tests__/clientWireSeam.test.ts` here, `src/server/createBatchingAdapter.test.ts` there).                                                                                                              |
