...
export function DateRangeControl(props: StudioFilterDateRangeControlProps) {
  ...
  // Handle to the debounced `onApply` scheduled by `scheduleApply` below. Declared here because the
  // external-sync effect has to cancel it too, and the blur handlers read it.
  ...
  React.useEffect(() => {
    // An EXTERNAL change to this filter — another surface's Clear (the drawer's
