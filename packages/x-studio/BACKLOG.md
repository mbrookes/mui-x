...
BL-193: Bring `examples/x-studio-ag` into the BL-189 typecheck gate. It's the one x-studio example still excluded from `pnpm-workspace.yaml` (`!examples/x-studio-ag`) because, unlike the other variants (which were the incomplete-tsconfig cascade and are now fixed + gated), it has **genuine** type errors (≈1306) — its ag-grid integration, not just the tsconfig. Apply the `extends ../../tsconfig.json` + augmentation-shim recipe first to clear the cascade, then triage and fix the real ag-grid type errors, then drop the `!examples/x-studio-ag` exclusion. Keep it out until it's actually green so the gate never reddens.
