...
`query_data_source` and the data-analysis tools (`describe_data_source`, `get_field_values`, `compute_field_stats`) are only registered when `data.queryDataSource` is configured — without it, they're omitted from `tools/list` entirely rather than advertised and left to fail.
