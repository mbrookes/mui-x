...
✅ BL-193: Bring `examples/x-studio-ag` into the BL-189 typecheck gate. It was the one x-studio example still excluded from `pnpm-workspace.yaml` (`!examples/x-studio-ag`), assumed to have **genuine** ag-grid integration type errors (≈1306). **Done** (a694b7d0e9): the prior diagnosis was wrong — **all 1306 errors were the same incomplete-tsconfig cascade** as the other variants, not genuine ag-grid drift. `x-studio-ag` consumes ag-grid only via the pre-built `ag-studio-react` package (which ships its own `.d.ts`, so it is _not_ typechecked from source); the example's own `src/**` had **zero** real type errors. Applying the standard `extends ../../tsconfig.json` recipe + the `typecheckAugmentations.ts` shim took it **1306 → 0** with no source edits and no `@ts-expect-error`. Dropped the `!examples/x-studio-ag` exclusion (membership is now plain `examples/x-studio*`); `cd examples/x-studio-ag && pnpm exec tsc --noEmit -p tsconfig.json` → 0. Sanity-checked `x-studio-composed` (still 0) and `pnpm --filter @mui/x-studio typescript` (still 0). No deps changed.

✅ BL-190: Clear the ~68 pre-existing eslint errors in x-studio — mostly legitimate German/French curly quotes (`„…"`) in the locale files tripping `mui/straight-quotes`, plus JSDoc/react-hooks-deps debt in `StudioUIConfigContext.ts`. The package is not lint-clean independent of recent changes; decide per-rule whether to fix or scope-disable (e.g. allow non-ASCII quotes in locale files). **Done** (500277ecdd): the real count was **158 errors** (the ~68 estimate undercounted; the spread was JSDoc-dominated, not quote-dominated), now **0**. `npx eslint packages/x-studio/src --ext .ts,.tsx` → 0 errors; `pnpm --filter @mui/x-studio typescript` clean; 1078 unit tests pass (locale `locales.test.ts` unchanged — translation strings byte-for-byte untouched, verified via empty `git diff` on `src/locales/`).
