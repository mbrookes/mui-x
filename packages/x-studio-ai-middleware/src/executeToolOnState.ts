...
// The generic value caps and the request trust boundary, both extracted from this file.
// `capIncomingDashboardState` and its thirty `MAX_STATE_*` constants were the larger half of a
// boundary whose other half already lived in `handleAIChat.ts`; the caps below stayed shared
// because a title or a filter value needs bounding wherever it arrives from.
import {
  capString,
  capTitle,
  capEntityId,
  ...
  MAX_LAYOUT_ROWS,
  MAX_DISTINCT_VALUES_IN_STATE_OUTPUT,
